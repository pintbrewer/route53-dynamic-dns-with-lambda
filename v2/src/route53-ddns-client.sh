...
myPublicIP=$(curl -q --$ipVersion -s  -H "x-api-key: $apiKey" "$myAPIURL" | jq -r '.return_message //empty')
...
elif [ "$listHosts" != "true" ]; then
    reply=$(curl -q --$ipVersion -s -H "x-api-key: $apiKey" -H "Content-Type: application/json" -d "{\"set_hostname\": \"${myHostname}\", \"validation_hash\": \"${myHash}\"}" "$myAPIURL")
else
    reply=$(curl -q --$ipVersion -s -H "x-api-key: $apiKey" -H "Content-Type: application/json" -d "{\"set_hostname\": \"${myHostname}\", \"validation_hash\": \"${myHash}\"}" "$myAPIURL")
fi
